...
def test_proxy_reload_failure_exits_nonzero(tmp_path: Path, monkeypatch) -> None:
    config_base = tmp_path / "config"
    state_base = tmp_path / "state"
    env = {
        "XDG_CONFIG_HOME": str(config_base),
    ...
    monkeypatch.setattr(
        cli,
        "reload_proxy",
        lambda profile: (["abc123"], [("abc123", "boom")]),
    )

    result = runner.invoke(cli.cli, ["proxy", "reload", "dev"], env=env)
    assert result.exit_code != 0
    assert "boom" in result.output
    assert "Proxy reload failed" in result.output
